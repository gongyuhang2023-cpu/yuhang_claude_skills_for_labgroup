...
**用途**：把活交到实验室的共享 GPU 服务器上跑之前，先回答三个问题 —— **这活该本地跑还是上服务器**、**要申请多少资源**、**现在交还是等**。然后生成 sbatch 脚本并提交。

**触发词**：`/spark-advisor`、`/spark-advisor debug`、`把作业交到服务器`、`这活本地还是服务器跑`、`要多少内存/显存`、`服务器忙不忙`

**为什么需要它**：这台机器是**统一内存**架构、多人共享一块卡。申报的 `--mem` 不是"上限"而是"被对账的申报值"——报低了会在内存吃紧时**第一个被冻**；`--cpus` 不写会静默落到 1 核，把 MSA/BLAS 全串行掉，**没有任何告警**。这些判据都写死在 skill 里，不用每次自己想。

**两条使用路径**：

| 场景 | 走哪个 |
|------|--------|
| 提交完就关电脑走人的活 | 默认流程：判执行地 → 定参数 → 生成 sbatch → 提交 → `wait` 盯结果 |
| **要盯着结果改代码、反复跑的调试循环** | **`/spark-advisor debug`** —— 开一个交互式会话，**排一次队，之后每次运行零等待**（实测每次启动 33–61 ms，而同一个作业走排队曾等 2 小时 51 分） |

**其它子命令**：`debug list`（看会话还剩多久）、`debug end`（收工释放）、`status`（只看机器忙不忙，不提交任何东西）。

**几个已经踩过的坑，skill 里都设了硬规则**：
- GPU / 重活**只能走队列**，绝不 `ssh 'python xxx.py'` 裸跑（裸跑不留痕、逃过内存守护、抢显存撞崩别人的卡）
- 作业"在跑但没进度"时**禁止直接 `scancel`** —— 对已冻结的作业 scancel 会让它永远卡在 `COMPLETING` 且不还内存，曾把整台机器堵三小时
- 调试会话**必须设时限**：闲置看门狗对交互式作业是豁免的，忘了收工没有任何东西会来回收

**依赖**：无（纯 Python 标准库 + `ssh`）。首次用需要 `advise.py init` 生成 `config.json` 填服务器连接信息 —— **该文件不入库**，仓库里只有 `config.example.json`。

**适用范围**：绑定本实验室的 Slurm 服务器（ARM64 + 统一内存 + 单卡共享）。换一台机器需要重新标定参数策略。
