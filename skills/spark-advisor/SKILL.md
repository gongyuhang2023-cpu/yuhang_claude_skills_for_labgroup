---
name: spark-advisor
description: 共享 GPU 服务器（Slurm/ARM64）作业顾问：先判任务该本地还是上服务器跑，上服务器的再定 --mem/--gres/--time 并可提交，避免统一内存 OOM；要盯着结果改代码、反复跑的**调试循环**改走交互式会话，排一次队之后每次运行零等待。触发：把作业交到 spark/服务器、这活本地还是服务器跑、要多少内存/显存、服务器忙不忙、该排队还是能跑、推荐 slurm 参数、在服务器上调试、要反复跑反复改、开个调试会话、每跑一次都要重新排队。
---
