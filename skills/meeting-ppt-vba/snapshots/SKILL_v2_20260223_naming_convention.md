...
**⚠️ 编码规则（关键）**：
- **中文 Windows 的 VBA 编辑器使用 GBK 编码**，不支持 UTF-8
- VBA 源码中**禁止直接使用 GBK 不兼容的 Unicode 字符**，包括：
  ...
  - `≈`（约等于）→ 用 `~` 或 `ChrW(8776)`
  - `""`（弯引号）→ 用直引号 `""`
- **推荐方案**：VBA 代码中文本内容全部使用英文 + ASCII 安全字符
- 中文标题如需出现，使用 `ChrW()` 逐字拼接（但不推荐，可读性差）
- 保存 .bas 文件时用 GBK 编码：`content.encode('gbk')`
- 复制到剪贴板前验证：`content.encode('gbk')` 不报错
